...
namespace XRMultiplayer
{
    ...
    public class NetworkPhysicsInteractable : NetworkBaseInteractable
    {
        ...
        {
            CustomDebugLog.Singleton.LogNetworkManager("CLIENT TO SERVER: REQUESTED Ownership");
            NetworkObject.ChangeOwnership(clientId);

            if (IsServer) {
                    CustomDebugLog.Singleton.LogNetworkManager("SERVER SAYS in server rpc: CHANGE THE OWNER");
                } else {
                    CustomDebugLog.Singleton.LogNetworkManager("CLIENT pieps in physics request ownership rpc: CHANGE THE OWNER pls but no right");
                }   
        }
