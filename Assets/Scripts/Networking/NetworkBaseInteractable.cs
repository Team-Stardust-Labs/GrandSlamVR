...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        void ResetObjectToHostServerRpc()
        {
            if (NetworkObject.OwnerClientId != NetworkManager.Singleton.LocalClientId) {
                ...
                if (IsServer) {
                    CustomDebugLog.Singleton.LogNetworkManager("SERVER SAYS: CHANGE THE OWNER");
                } else {
                    CustomDebugLog.Singleton.LogNetworkManager("CLIENT pieps: CHANGE THE OWNER pls but no right");
                }   
            }
        }
        ...
        IEnumerator CheckForOwnerInteraction()
        {
            while (IsOwner)
            {
                if (isInteracting)
                {
                    ...
                    {
                        CustomDebugLog.Singleton.LogNetworkManager($"Interacting is true and either selected is false or selected is a socket on object {gameObject.name}. Is this intentional?");
                        m_IsInteracting.Value = false;
                    }
                }
